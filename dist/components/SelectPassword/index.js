...
export const SelectPassword = ({ label = "Mot de passe", description, password = "", onChange, passwordConfig = {
    ...
    return (_jsxs("div", { className: "flex flex-col gap-4 w-full", children: [_jsx(Input, { "data-test": testId, label: label, value: value, type: showPassword ? "text" : "password", onValueChange: (val) => {
                    setValue(val);
                    if (onChange)
                        onChange(val);
                }, description: description, classNames: {
                    input: "font-mono",
