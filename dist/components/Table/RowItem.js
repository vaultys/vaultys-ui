...
export const RowItem = ({ type, value, title, editMode, items, onChange, onClick, className, children, disabled = false, maxLines, inputType = "text", id, testId, }) => {
    ...
    return (_jsx("td", { className: ` px-2 first:rounded-l-lg first:rtl:rounded-r-lg first:rtl:rounded-l-[unset] last:rounded-r-lg last:rtl:rounded-l-lg last:rtl:rounded-r-[unset] ${className}`, id: id, "data-testid": testId, children: getChild() }));
};
